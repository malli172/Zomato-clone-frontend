  ...
  return (
    <div class="tab-options">
      <div className='max-width options-wrapper'>
        {tabs.map((tab) => {
          return(
            <div onClick={() => setactive(tab.name)}
            className={`tab-item   absolute-center cur-po ${active === tab.name && "active"}`}
            >
              <div className='tab-image-container absolute-center' style={{backgroundColor:`${active===tab.name?tab.backdrop:""}`}}>
                <img className='tab-image' alt={tab.name} src={active===tab.name ? tab.active_img:tab.inactive_img}></img>
              </div>
